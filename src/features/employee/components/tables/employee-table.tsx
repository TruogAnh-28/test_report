...
import {
  type SearchEmployees, type Employee,
...
export interface EmployeesTableProps extends BaseDataTableProps, TableToolbarProps {
  data?: Employee[]
...
export function EmployeesTable({
  ...
  const columns = React.useMemo<ColumnDef<Employee>[]>(
    () => {
      const selectColumn: ColumnDef<Employee>[] = hasSelection
        ? []
        : []
      return [
        ...selectColumn,
        ...
        },
        {
          ...
          cell: ({ row }) => (
            <div className="flex gap-3 items-center">
              ...
              {
                row.original.gender === -1
                  ? "Chưa xác định"
                  : row.original.gender === 1
                    ? "Nam"
                    : row.original.gender === 2
                      ? "Nữ"
                      : "Khác"
              }
