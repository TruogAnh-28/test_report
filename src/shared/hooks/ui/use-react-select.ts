...
const SELECT_CLEAR: unique symbol = Symbol(process.env.NODE_ENV !== "production" ? "CLEAR" : "",)
...
function useSingle<V extends SelectValue>(props?: SelectProps<V>) {
  ...
  const [
    selected,
    setSelected,
  ] = useControllableState<V | undefined>({
  ...
  const ctx = React.useMemo(
    () => {
      const isSelected = (triggerValue?: TriggerValue<V>) => {
      ...
      return {
        ...
        setSelected,
        isSelected,
        onSelect: (triggerValue: TriggerValue<V>) => {
...
export interface ReturnSelect<V extends SelectValue, P extends SelectProps<V>> {
  // for CommandItem
  ...
  setSelected: P extends SelectSingleProps<V>
    ? React.Dispatch<React.SetStateAction<SelectSingleProps<V>["value"]>>
    : React.Dispatch<React.SetStateAction<SelectMultipleProps<V>["value"]>>
}
...
>(props?: P): ReturnSelect<V, P> {
  const {
    selected, isSelected, onSelect, setSelected,
  } = useSelection<V>(props)
  ...
  const getSelectItemProps = React.useCallback(
    function <Pr>(props: Pr & { value?: TriggerValue<V>
      disabled?: boolean }) {
      const {
        value, disabled: disabledProp = false,
      } = props ?? {
      }
