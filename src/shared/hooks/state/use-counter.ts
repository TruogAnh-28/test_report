...
export function useCounter(props: UseCounterProps): UseCounterReturn {
  const {
    min, max, defaultValue = 0, onValueChange, shouldUpdate, value,
  } = props ?? {
  }
  ...
  if (typeof min === "number") {
    defaultValueRef.current = Math.max(
      defaultValueRef.current, min
    )
  }

  if (typeof max === "number") {
    defaultValueRef.current = Math.min(
      defaultValueRef.current, max
    )
  }
      ...
      return {
        ...
        inc: (delta: React.SetStateAction<Value> = 1) => {
          ...
          // eslint-disable-next-line no-console
            console.error("delta has to be a number or function returning a number, got "
              + typeof rDelta,)
          }
        ...
        reset: (value: React.SetStateAction<Value>) => {
          ...
          // eslint-disable-next-line no-console
            console.error("value has to be a number or function returning a number, got "
              + typeof rValue,)
          }
