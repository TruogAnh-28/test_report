...
type GetValidNumberConfig = { max: number
  min?: number
  loop?: boolean }
...
export function getValidNumber(
  ...
  if (!isNaN(numericValue)) {
    ...
    return numericValue.toString().padStart(
      2, "0"
    )
  }
...
export function getValidArrowHour(
  value: string, step: number
) {
  return getValidArrowNumber(
    value, {
      min: 0,
      max: 23,
      step,
...
export function getValidArrow12Hour(
  value: string, step: number
) {
  return getValidArrowNumber(
    value, {
...
export function set12Hours(
  date: Date, value: string, period: Period
) {
  const hours = parseInt(
    getValid12Hour(value), 10
  )
...
export function getArrowByType(
  ...
  switch (type) {
    case "minutes":
      return getValidArrowMinuteOrSecond(
        value, step
      )
    case "seconds":
      return getValidArrowMinuteOrSecond(
    ...
    case "hours":
      return getValidArrowHour(
        value, step
      )
    case "12hours":
      return getValidArrow12Hour(
        value, step
      )
    default:
      return "00"
